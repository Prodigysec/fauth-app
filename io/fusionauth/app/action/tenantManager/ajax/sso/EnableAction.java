...
@List({@Forward(code = "input", page = "/tenant-manager/ajax/sso/enable.ftl")})
...
public class EnableAction extends BaseTenantManagerAction {
  ...
  public String post() {
    ...
    this.delegate.execute(paramFusionAuthClient -> paramFusionAuthClient.patchIdentityProvider(this.identityProviderId, Map.of("identityProvider", Map.of("enabled", Boolean.valueOf(true)))));
    writeAuditLog("Enabled the Tenant Manager IdP Configuration with Id [" + String.valueOf(this.identityProviderId) + "]");
    return "success";
  }
}
