...
public class IndexAction extends BaseSearchAction<Application, ApplicationSearchCriteria> {
  ...
  protected ApplicationSearchCriteria defaultSearchCriteria() {
    return (new ApplicationSearchCriteria()).with(paramApplicationSearchCriteria -> paramApplicationSearchCriteria.state = ObjectState.Active);
  }
  ...
  protected SearchResults<Application> search() {
    ...
    if (uUID != null) {
      ...
      if (clientResponse.wasSuccessful() && ((ApplicationResponse)clientResponse.getSuccessResponse()).application.state == ObjectState.Active)
        return new SearchResults<>(List.of(((ApplicationResponse)clientResponse.getSuccessResponse()).application), 1L); 
      return null;
    } 
    this.s.state = ObjectState.Active;
