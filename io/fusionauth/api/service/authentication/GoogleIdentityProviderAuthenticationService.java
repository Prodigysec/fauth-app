...
public class GoogleIdentityProviderAuthenticationService extends BaseIdentityProviderAuthenticationService {
  ...
  public AuthenticationService.AuthenticationResult _login(Tenant paramTenant, Application paramApplication, @Nonnull BaseIdentityProvider<?> paramBaseIdentityProvider, IdentityProviderLoginRequest paramIdentityProviderLoginRequest, @Nullable ExternalIdentifier paramExternalIdentifier) {
    ...
    if (str == null) {
      ...
      debugger.log("Using code and redirect_uri with login method " + String.valueOf(googleIdentityProvider.lookupLoginMethod(paramIdentityProviderLoginRequest.applicationId)) + ":");
      JsonNode jsonNode1 = callAccessTokenAPI(debugger, googleIdentityProvider, paramIdentityProviderLoginRequest.applicationId, str1, str2);
      str = jsonNode1.at("/id_token").asText();
    } else {
      debugger.log("Using token with login method " + String.valueOf(googleIdentityProvider.lookupLoginMethod(paramIdentityProviderLoginRequest.applicationId)) + ":");
    } 
  ...
  private JsonNode callAccessTokenAPI(Debugger paramDebugger, GoogleIdentityProvider paramGoogleIdentityProvider, UUID paramUUID, String paramString1, String paramString2) {
    return makeRequest(paramDebugger, "https://oauth2.googleapis.com/token", paramRESTClient -> paramRESTClient.bodyHandler((RESTClient.BodyHandler)(new FormDataBodyHandler()).withParameter("client_id", paramGoogleIdentityProvider.lookupClientId(paramUUID)).withParameter("client_secret", paramGoogleIdentityProvider.lookupClientSecret(paramUUID)).withParameter("code", paramString1).withParameter("grant_type", GrantType.authorization_code.name()).withParameter("redirect_uri", paramString2)), ExternalAuthenticationException.Reason.GoogleToken, RESTClient.HTTPMethod.POST);
  }
  
  private JsonNode callTokenInfoAPI(Debugger paramDebugger, String paramString) {
    return makeRequest(paramDebugger, "https://www.googleapis.com/oauth2/v3/tokeninfo", paramRESTClient -> paramRESTClient.urlParameter("id_token", paramString), ExternalAuthenticationException.Reason.GoogleTokenInfo, RESTClient.HTTPMethod.GET);
  }
}
